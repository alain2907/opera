...
export default function BilanPassif() {
  ...
  const getLigneStyle = (niveau: number, montant: number) => {
    const baseStyle = 'py-2 border-b border-gray-200 hover:bg-green-50 cursor-pointer';

    if (niveau === 1) {
      return `${baseStyle} bg-green-50 font-bold text-lg`;
    }
    if (niveau === 2) {
  ...
  return (
    <>
      ...
      <div className="mb-6 flex justify-between items-center">
        <h1 className="text-3xl font-bold">Bilan Passif</h1>
        <button
          onClick={() => router.push('/bilan-actif')}
      ...
      <div className="bg-white rounded shadow overflow-hidden">
        <div className="bg-green-600 text-white p-4 flex justify-between items-center">
          <h2 className="text-xl font-bold">PASSIF</h2>
          <div className="text-xl font-bold">{formaterMontant(bilan?.total_passif || 0)}</div>
        </div>
              ...
              {!editMode ? (
                <div className="flex items-center gap-4">
                  ...
                  <button
                    ...
                    className="text-xs px-3 py-1 bg-white text-green-600 rounded hover:bg-gray-100"
                  >
                    Modifier
                  </button>
                </div>
              ...
              {detailLigne.comptes.length === 0 && (
                <div className="text-center py-8">
                  <p className="text-gray-500 mb-4">
                    Aucun compte trouvé dans le plan comptable
                  </p>
                  <p className="text-sm text-gray-600 mb-6">
                    💡 Cliquez sur <strong>"Modifier"</strong> en haut pour changer la plage de comptes recherchés
                  </p>
                  <button
