...
export default function BilanPassifPWA() {
  ...
  const getLigneStyle = (niveau: number, montant: number) => {
    ...
    if (niveau === 0) {
      // Titres principaux (CAPITAUX PROPRES, DETTES, etc.)
      return `${baseStyle} bg-purple-100 font-bold text-lg text-purple-900`;
    }

    if (niveau === 1) {
      // Sous-titres et totaux
      return `${baseStyle} bg-purple-50 font-semibold text-purple-800`;
    }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-50">
        ...
        <div className="flex items-center justify-center h-screen">
          <div className="text-xl">Chargement du bilan passif...</div>
        </div>
      </div>
    );
  }
  ...
  // Filter only PASSIF section
  const lignesPassif = bilan.passif;
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-50">
      ...
      <div className="max-w-7xl mx-auto p-6 pt-24">
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {lignesPassif.map((ligne, index) => (
                <tr
                  key={`passif-${index}`}
                  className={`${getLigneStyle(ligne.niveau, ligne.montant_n)} cursor-pointer hover:bg-purple-100`}
              ...
              <tr className="bg-gradient-to-r from-purple-100 to-purple-200 border-t-4 border-purple-400">
                <td className="px-4 py-4 font-bold text-2xl text-purple-900">
                  TOTAL GÉNÉRAL PASSIF
                </td>
                <td className="px-4 py-4 text-center text-xs font-bold">EE</td>
                <td className="px-4 py-4 text-right font-mono font-bold text-2xl text-purple-900">
                  {formaterMontant(bilan.total_passif)}
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <div className="flex gap-4 mb-6">
          <button
            onClick={() => router.push('/pwa/bilan-actif')}
                  ...
                  {detailLigne.comptes_specifiques ? (
                    ...
                    <>
                      ...
                      <button
                        ...
                        className="text-xs px-3 py-1 bg-white text-purple-600 rounded hover:bg-gray-100"
                      >
                        Modifier
                      </button>
                      <button
